...
header('location:index.php');
	...
	if(isset($allocqation[0]) && $auser[0]["user_type"] !== "manager")
	{
		$allocated_list = $conn->getRow("allocqation",["date" => $dat]);
		foreach($allocated_list as $row)
		{
			$lsuser = $conn->getRow("users",["id" => $row["user_id"]]);
			if (isset($lsuser[0])) {
				if($lsuser[0]["user_type"] == "manager")
				{
					$manager = $lsuser[0];
				}
		...
		echo "isset manager ".isset($manager);
	}
