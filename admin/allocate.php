...
if(isset($_POST['submit']))
  {
	...
	$in = $conn->insData("allocqation",["user_id" => $guard,
								 "note" => $note,
								 "date" => $date,
								 "site_id" => $site_id,
								 "shift_id" => $shift]);

	$allo_user = $conn->getRow("users",["id"=>$guard]);
	if($allo_user[0]["user_type"] !== "manager")
	{
		$checkpoints = $conn->getRow("checkpoint",["site_id"=>$site_id]);
		$cp_data= "[";
		foreach($checkpoints as $row)
		{
			$cp_data = $cp_data."{''cp_code'':''".$row["code"]."'',''lastcheck'':''0'',''data'':[''no'',''no'',''no'',''no'',''no'',''no'',''no'',''no'',''no'',''no'',''no'',''no'',''no'']},";
		}
		$cp_data = rtrim($cp_data,",");
		$cp_data = $cp_data."]";
		echo $cp_data;
		$ua = $conn->insData("user_checkpoint",["allocqation_id"=>$in,"data"=>$cp_data,"date"=>date("Y-m-d")]);
	}
	
	$msg="Information Updated Successfully shift =".$shift;
	...
	<?php include('includes/leftbar.php');?>
		<div class="content-wrapper">
			<div class="container-fluid">
				<div class="row">
					<div class="col-md-12">
						<h3 class="page-title">Allocate Guard </h3>
						<div class="row">
							<div class="col-md-12">
...
<div class="form-group">
<label class="col-sm-2 control-label">Select Shift<span style="color:red">*</span></label>
<div class="col-sm-4">
...
foreach($results as $result)
{				?>	
                            <option value="<?php echo $result->id;?>"><?php echo htmlentities($result->shift_type);?></option>
<?php $cnt=$cnt+1; }} ?>
                            </select>
